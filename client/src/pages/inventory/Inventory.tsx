...
const Inventory: React.FC = () => {
  ...
  const [selectedTypeForEdit, setSelectedTypeForEdit] = useState<string>('');
  ...
  const openEditPopup = (item: InventoryItem) => {
    ...
    setSelectedTypeForEdit(item.type);
  };
  ...
  const handleEditConfirm = async () => {
    ...
    try {
      await axios.put(import.meta.env.VITE_API_URL + `/api/inventory/${editingItem._id}`, {
          ...
          <section className={styles.inventoryAdd}>
            ...
            <form onSubmit={handleAdd} className={styles.addForm}>
              ...
              >
                ...
                {(types.find((t) => t.name === newType)?.sub_types || []).map((st) => (
        ...
        {items.map((item) => (
          ...
          >
            <div className={styles.itemInfo}>
              ...
              <img src={`/images/${item.name}.png`} alt="img" className={styles.itemInfo_img} />
            ...
            <div className={styles.itemActions}>
              <div className={styles.itemActions_quantity}>
                {item.quantity}
              </div>
              <button onClick={() => openEditPopup(item)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M5 19H6.425L16.2 9.225L14.775 7.8L5 17.575V19ZM4 21C3.71667 21 3.47933 20.904 3.288 20.712C3.09667 20.52 3.00067 20.2827 3 20V17.575C3 17.3083 3.05 17.054 3.15 16.812C3.25 16.57 3.39167 16.3577 3.575 16.175L16.2 3.575C16.4 3.39167 16.621 3.25 16.863 3.15C17.105 3.05 17.359 3 17.625 3C17.891 3 18.1493 3.05 18.4 3.15C18.6507 3.25 18.8673 3.4 19.05 3.6L20.425 5C20.625 5.18333 20.7707 5.4 20.862 5.65C20.9533 5.9 20.9993 6.15 21 6.4C21 6.66667 20.954 6.921 20.862 7.163C20.77 7.405 20.6243 7.62567 20.425 7.825L7.825 20.425C7.64167 20.6083 7.429 20.75 7.187 20.85C6.945 20.95 6.691 21 6.425 21H4ZM15.475 8.525L14.775 7.8L16.2 9.225L15.475 8.525Z" fill="white"/>
                </svg>

              </button>
              <button onClick={() => handleDelete(item._id)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4 7H20M10 11V17M14 11V17M5 7L6 19C6 19.5304 6.21071 20.0391 6.58579 20.4142C6.96086 20.7893 7.46957 21 8 21H16C16.5304 21 17.0391 20.7893 17.4142 20.4142C17.7893 20.0391 18 19.5304 18 19L19 7M9 7V4C9 3.73478 9.10536 3.48043 9.29289 3.29289C9.48043 3.10536 9.73478 3 10 3H14C14.2652 3 14.5196 3.10536 14.7071 3.29289C14.8946 3.48043 15 3.73478 15 4V7" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
        ...
        <div className={styles.popupOverlay}>
          <div className={styles.popupContent}>
            ...
            <div className={styles.editRow}>
              ...
              >
                <option value="">Alege subtip</option>
                {(types.find((t) => t.name === editType)?.sub_types || []).map((st) => (
                  <option key={st} value={st}>
                    {st}
                  </option>
                ))}
...
<div className={styles.bottom}>
  <button className={styles.bottom_cart}>Cart</button>
  <button className={styles.bottom_use}>Use</button>
</div>
    </div>
  );
};
